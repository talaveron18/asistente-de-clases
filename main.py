...
class AsistenteClasesApp(ctk.CTk):
    ...
    def _crear_interfaz(self):
        ...
        ctk.CTkLabel(
            cabecera,
            text="Asistente de Clases",
            font=ctk.CTkFont(size=20, weight="bold"),
        ).pack(side="left", padx=18)
        self.estado_modelos = ctk.CTkLabel(
            cabecera, text="Preparando...", text_color="#ffb300"
        )
    ...
    def _campos_clase(self, parent):
        ...
        ctk.CTkLabel(marco, text="Materia").grid(
            row=0, column=0, padx=(12, 6), pady=10
        )
        materia = ctk.CTkComboBox(
            marco, values=self.repositorio.materias() or [""], width=260
        )
        materia.grid(row=0, column=1, padx=6, pady=10, sticky="ew")
        materia.set(self.config_obj.ultima_materia or "")
        ctk.CTkLabel(marco, text="Título").grid(
            row=0, column=2, padx=(18, 6), pady=10
        )
    ...
    def _tab_grabacion(self):
        ...
        ctk.CTkLabel(panel, text="Micrófono:").pack(
            side="left", padx=(12, 6), pady=12
        )
        self.combo_micro = ctk.CTkComboBox(panel, width=430)
        self.combo_micro.pack(side="left", padx=6)
        valores = [
            f"{i}: {nombre}" for i, nombre in GrabadorAudio.listar_dispositivos()
        ] or ["Sin dispositivos detectados"]
        ...
        self.btn_detener = ctk.CTkButton(
            controles,
            text="Detener, transcribir y guardar",
        ...
        self.reloj = ctk.CTkLabel(
            controles, text="00:00:00", font=ctk.CTkFont(size=20, weight="bold")
        )
        ...
        self.texto_grabar = ctk.CTkTextbox(
            self.tab_grabar, font=ctk.CTkFont(size=13)
        )
    ...
    def _tab_archivo_multimedia(self):
        ...
        self.etiqueta_archivo = ctk.CTkLabel(
            panel, text="Ningún audio o vídeo seleccionado", anchor="w"
        )
        self.etiqueta_archivo.pack(
            side="left", fill="x", expand=True, padx=12, pady=12
        )
        ctk.CTkButton(
            panel, text="Seleccionar audio o vídeo", command=self._seleccionar_archivo
        ).pack(side="left", padx=6)
        ctk.CTkButton(
            panel, text="Transcribir y guardar", command=self._transcribir_archivo
        ).pack(side="left", padx=(0, 12))

        ctk.CTkLabel(
            self.tab_archivo,
            text=(
                "Admite vídeos largos (MP4, MKV, MOV, AVI, WEBM…). "
                "Solo se usa su audio y el vídeo original no se copia al repositorio."
            ),
            text_color="#aaaaaa",
            anchor="w",
        ...
        self.texto_archivo = ctk.CTkTextbox(
            self.tab_archivo, font=ctk.CTkFont(size=13)
        )
    ...
    def _tab_biblioteca_clases(self):
        ...
        self.buscar = ctk.CTkEntry(
            superior, placeholder_text="Buscar por materia, título o fecha..."
        )
        self.buscar.pack(side="left", fill="x", expand=True, padx=(12, 6), pady=12)
        ctk.CTkButton(
            superior, text="Buscar", command=self._refrescar_biblioteca, width=90
        ).pack(side="left", padx=6)
        ctk.CTkButton(
            superior,
            text="Abrir carpeta general",
    ...
    def _refrescar_biblioteca(self):
        ...
        if not clases:
            ctk.CTkLabel(
                self.lista_clases, text="Todavía no hay clases guardadas."
            ).pack(pady=30)
            return
        for clase in clases:
            ...
            texto = (
                f"{clase.get('numero', 0):03d} · {clase.get('materia', '')} · "
                f"{clase.get('titulo', '')} · {fecha}"
            )
            ctk.CTkLabel(fila, text=texto, anchor="w").pack(
    ...
    def _tab_configuracion(self):
        ...
        ctk.CTkLabel(
            marco, text="Token de Hugging Face (opcional)", anchor="w"
        ...
        self.modelo = ctk.CTkComboBox(
            marco, values=["tiny", "base", "small", "medium", "large-v3"]
        )
        ...
        self.idioma = ctk.CTkComboBox(
            marco, values=["es", "en", "fr", "de", "it", "pt", "auto"]
        )
        self.idioma.pack(fill="x", pady=(4, 12))
        self.idioma.set(self.config_obj.idioma)
        self.gpu = ctk.CTkSwitch(
            marco, text="Usar GPU NVIDIA cuando esté disponible"
        )
        ...
        ctk.CTkButton(
            marco,
            text="Guardar y recargar modelos",
    ...
    def _datos_clase(self, materia, titulo):
        ...
        if not m or not t:
            messagebox.showwarning(
                "Datos de la clase", "Indica la materia y el título antes de continuar."
            )
            return None
        return m, t
    ...
    def _iniciar_grabacion(self):
        ...
        self.grabador = GrabadorAudio(
            self.config_obj.sample_rate, self._indice_microfono()
        )
        ...
        if not ok:
            messagebox.showerror(
                "Audio", self.grabador.ultimo_error or "No se pudo iniciar la grabación."
            )
            return
    ...
    def _seleccionar_archivo(self):
        ruta = filedialog.askopenfilename(
            ...
            filetypes=[
                (
                    "Audio y vídeo",
                    "*.wav *.mp3 *.m4a *.flac *.ogg *.wma *.aac *.opus "
                    "*.mp4 *.mkv *.mov *.avi *.webm *.m4v *.wmv *.mpeg *.mpg *.ts",
                ),
        ...
        if ruta:
            ...
            clase_tipo = tipo_archivo(ruta)
            etiqueta = "Vídeo" if clase_tipo == "video" else "Audio"
    ...
    def _procesar(self, ruta, tab, materia, titulo):
        ...
        def worker():
            ...
            tipo_original = "audio"
            try:
                ruta_procesable, temporal, tipo_original = preparar_para_transcripcion(
                    ruta, callback_media
                )
                segmentos = self.transcriptor.transcribir_archivo(
                    ruta_procesable,
                    callback_progreso=lambda msg, p: self.after(
                        0, self._progreso, barra, msg, max(0.08, p)
                    ),
                    min_hablantes=self.config_obj.min_hablantes,
                    max_hablantes=self.config_obj.max_hablantes,
                )
                # Las grabaciones y los audios importados se archivan. Los vídeos no:
                # pueden ocupar muchos GB y el usuario conserva el original.
                fuente_archivable = ruta if tipo_original == "audio" else None
                carpeta = self.repositorio.guardar_clase(
                    materia, titulo, segmentos, fuente_archivable
                )
    ...
    def _mostrar_resultados(self, caja, segmentos, carpeta):
        ...
        self._refrescar_biblioteca()
        messagebox.showinfo(
            "Clase guardada", f"Se ha archivado correctamente en:\n{carpeta}"
        )
    ...
    def _cargar_modelos(self):
        self.estado_modelos.configure(
            text="Cargando modelos...", text_color="#ffb300"
        )
        self.estado.configure(
            text="La primera carga descargará el modelo automáticamente."
        )
        ...
        def worker():
            try:
                ...
                motor.cargar_modelos(
                    lambda msg, p: self.after(0, self.estado.configure, {"text": msg})
                )
                ...
                self.after(
                    0,
                    self.estado.configure,
                    {"text": "Listo para grabar o transcribir audio y vídeo."},
                )
            except Exception as exc:
                self.after(
                    0,
